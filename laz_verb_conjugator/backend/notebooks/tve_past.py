...
def conjugate_past(infinitive, subject=None, obj=None, applicative=False, causative=False, simple_causative=False, use_optional_preverb=False):
    ...
    for third_person, region_str in third_person_forms:
        ...
        for region in regions_for_form:
            region = region.strip()
            personal_pronouns = get_personal_pronouns(region, 'tve_past')
            ...
            if applicative and causative:
                ...
                if root in (('şums', 'şups', 'dums', 'tkums', 'tkups')) or infinitive in ('meşǩvu'):  # remove "am/um/ups" endings from root, use infinitives for preverbs
                    ...
                    root = root[:-2]
                elif root.endswith(('umers', 'amers')):  # removed "root in ('dumers') as I am not sure what that was good for, can readd"
                    root = root[:-5] + 'v'
                else:
                    root = root
...
def format_imperatives(imperatives):
    ...
    for region, conjugations in imperatives.items():
        subject_pronouns = get_personal_pronouns(region, 'tve_past')
