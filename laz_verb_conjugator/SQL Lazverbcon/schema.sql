...
CREATE TABLE IF NOT EXISTS pronoun (
    dialect_id INT REFERENCES dialect(dialect_id),
    code person,
    frame frame_type,
    form TEXT NOT NULL,
    PRIMARY KEY (dialect_id, code, frame)
);
...
-- Groups the same verb lexeme across dialect variants (same meaning/etymology, different spellings)
-- Example: uqoun and uyonun
...
-- - root_infinitive (formerly known as preverb_family) groups preverb variants of the SAME underlying infinitive
--   (e.g., obaru, mebaru, ebaru share one root: root_infinitive).
...
CREATE TABLE IF NOT EXISTS verb (
  ...
  -- root_infinitive_id groups preverb variants (obaru/mebaru/ebaru...), separate from verb_family
  root_infinitive_id INTEGER REFERENCES root_infinitive(root_infinitive_id),
